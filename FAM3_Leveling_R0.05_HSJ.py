...
class MainThread(QObject):
    ...
    def __init__(self, debugFlag, date, debugDate, constDate, list_masterFile, moduleMaxCnt, emgHoldList, cb_round):
        super().__init__(),
    ...
    def run(self):
        ...
        try:
            ...
            df_SmtAssyInven = self.readDB('10.36.15.42',
                                    ...
                                    "SELECT INV_D, PARTS_NO, CURRENT_INV_QTY FROM pdsg0040 where INV_D = TO_DATE(" + str(yesterday) + ",'YYYYMMDD')")
            df_SmtAssyInven['현재수량'] = 0
            #2차 특수피킹 리스트 불러오기 HSJ r0.05
            if Path(self.list_masterFile[13]).is_file():
                df_secOrderOtherList = pd.read_excel(self.list_masterFile[13], skiprows=5)
...
class Ui_MainWindow(QMainWindow):
    ...
    def loadMasterFile(self):
        ...
        if os.path.exists(r'.\\input\\Master_File\\' + date + '\\' + roundTxt + r'\\100L1304(' + date[4:8] + ')OTHER_2차.xlsx'):
            secOtherListFilePath = r'.\\input\\Master_File\\' + date + '\\' + roundTxt + r'\\100L1304(' + date[4:8] + ')OTHER_2차.xlsx'
        else:
