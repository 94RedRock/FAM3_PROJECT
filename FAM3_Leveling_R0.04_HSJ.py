    ...
    def run(self):
        ...
        try:
            ...
            for i in df_switch.index:
                readDate = df_switch['Planned Prod. Completion date'][i]
                if oneMonthLater >= readDate:
            ...
            progress += round(maxPb / 20)
            self.spReturnPb.emit(progress)
            if not os.path.exists(f'.\\Output\\Result\\{str(today)}\\1차'):
                os.makedirs(f'.\\Output\\Result\\{str(today)}\\1차')
            if not os.path.exists(f'.\\Output\\Result\\{str(today)}\\2차'):
                os.makedirs(f'.\\Output\\Result\\{str(today)}\\2차')
            if self.cb_round ==  '1차':
                outputFile = f'.\\Output\\Result\\{str(today)}\\1차\\{str(today)}_Sp_1차.xlsx'
            if self.cb_round ==  '2차':
                outputFile = f'.\\Output\\Result\\{str(today)}\\2차\\{str(today)}_Sp_2차.xlsx'
            df_mergeOrderResult.to_excel(outputFile, index=False)
